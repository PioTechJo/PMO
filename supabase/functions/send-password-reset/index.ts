...
serve(async (req) => {
  ...
  try {
    ...
    // Only a Manager can force-reset someone else's password
    ...
    if (callerProfile?.role !== "Manager") {
      return new Response(JSON.stringify({ error: "Only a Manager can reset another user's password." }), {
        status: 403,
    ...
    const vars = { email, temp_password: newPassword, portal_url: PORTAL_URL };
